...
// TestUnsignedSearchRange tests looking up keys with
...
func TestUnsignedSearchRange(t *testing.T) {
	ranges := [][]uint16Range{
		uint16Range65535,
		uint16Range0_65535,
		uint16Range1_65534,
	...
	for _, r := range ranges {
		...
		t.Run(name, func(t *testing.T) {
			...
			for _, pr := range rangeLookupMap {
				var gotEntry string
				tu.Search(pr.prefix(), pr.start, func(prefix uint, _ uint16, v string) bool {
					gotEntry = v
					return true
				})
...
func BenchmarkTrieSearchRange(b *testing.B) {
	tri := NewUintTrie[uint32, *struct{}]()
	emptyS := &struct{}{}
	count := uint(0)
	// mimic adding 2 octets worth of addresses
	...
	b.ResetTimer()
	var st *struct{}
	tri.Search(16, 0xffff_0000, func(_ uint, _ uint32, v *struct{}) bool {
		st = v
		return true
	})
	if st == nil {
	...
	for i := uint32(0); i < 255; i++ {
		upperOct := i << 8
		var st *struct{}
		tri.Search(24, 0xffff_0000|upperOct, func(_ uint, _ uint32, v *struct{}) bool {
			st = v
			return true
		})
		if st == nil {
		...
		for t := uint32(0); t < 255; t++ {
			var st *struct{}
			tri.Search(32, 0xffff_0000|upperOct|t, func(_ uint, _ uint32, v *struct{}) bool {
				st = v
				return true
			})
			if st == nil {
...
func FuzzUint8(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, sequence []byte) {
		...
		for seenK, seenV := range seen {
			var val testEntry
			tree.Search(uint(seenV.plen), seenV.k, func(_ uint, _ uint8, v testEntry) bool {
				val = v
				return true
			})
