...
type Trie[K, T any] interface {
	...
	// the argument prefix-key pair. If the Search function argument
	// returns false the search will stop. Search will return
	...
	Search(prefix uint, key K, fn func(uint, K, T) bool)
...
// match. If the Search function argument returns false the search will
// stop.
//
...
// trie.
func (t *trie[K, T]) Search(prefixLen uint, k Key[K], fn func(prefix uint, key Key[K], value T) bool) {
	if k == nil {
		return
	}
